...
impl Hera {
    ...
    /// Uses the viseme backend (mascots, no `face_url`) or wav2lip backend
    /// (human faces, `face_url` set). Both are sovereign services on genesis
    /// (`WAV2LIP_URL` default `127.0.0.1:8012`, `VISEME_URL` default `127.0.0.1:8014`).
    pub async fn animate_avatar(
        ...
        // Save mp4 to the shared outputs dir (same dir generate_image uses for PNGs).
        let output_dir = generated_outputs_dir()?;
        fs::create_dir_all(&output_dir)?;
