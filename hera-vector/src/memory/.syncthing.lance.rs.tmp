...
impl LanceStore {
    ...
    pub async fn store(&self, vectors: Vec<f32>, payload: serde_json::Value) -> Result<()> {
        ...
        let batch = RecordBatch::try_new(schema.clone(), vec![id_array, list_array, payload_array])?;
    ...
    pub async fn search(&self, vector: Vec<f32>, limit: u64) -> Result<Vec<serde_json::Value>> {
        ...
        let stream = table.query().nearest_to(vector)?.limit(limit as usize).execute().await?;
