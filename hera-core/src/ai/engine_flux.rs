...
use candle_core::{IndexOp, Tensor, Module};
...
pub struct FluxEngine {
    ...
    ae_cfg: flux::autoencoder::Config,
...
impl FluxEngine {
    pub fn new() -> Result<Self> {
        ...
        let api = match std::env::var("HF_TOKEN") {
            Ok(token) => hf_hub::api::sync::ApiBuilder::new().with_token(Some(token)).build()?,
            Err(_) => hf_hub::api::sync::Api::new()?,
        };
        let bf_repo = api.repo(hf_hub::Repo::model("receptektas/black-forest-labs-ae_safetensors".to_string()));

        // 1. T5 Loading (Using Q8_0 Quantized GGUF Encoder-only model to save ~32GB of VRAM/Disk overhead)
        let t5_config_repo = api.repo(hf_hub::Repo::model("city96/t5-v1_1-xxl-encoder-bf16".to_string()));
        ...
        let t5_repo = api.repo(hf_hub::Repo::model("city96/t5-v1_1-xxl-encoder-gguf".to_string()));
        ...
        let t5_tokenizer_file = api.model("lmz/mt5-tokenizers".to_string()).get("t5-v1_1-xxl.tokenizer.json")?;
        ...
        let clip_model = clip::text_model::ClipTextTransformer::new(clip_vb.pp("text_model"), &clip_config)?;
        ...
        let flux_file = api.repo(hf_hub::Repo::model("lmz/candle-flux".to_string())).get("flux1-schnell.gguf")?;
        let flux_vb = candle_transformers::quantized_var_builder::VarBuilder::from_gguf(flux_file, &device)?;
    ...
    pub async fn generate_image(&self, prompt: &str, width: usize, height: usize) -> Result<Vec<u8>> {
        let dtype = self.device.bf16_default_to_f32();
        
        let mut t5_tokens = self.t5_tokenizer
            .encode(prompt, true)
        ...
        }).await.map_err(|e| anyhow::anyhow!("T5 Panic: {}", e))??;

        let clip_tokens = self.clip_tokenizer
            .encode(prompt, true)
            .map_err(|e| anyhow::anyhow!("{}", e))?
            .get_ids()
            .to_vec();
        ...
        let clip_emb = tokio::task::spawn_blocking(move || {
            ...
            model.forward(&tokens)
        }).await.map_err(|e| anyhow::anyhow!("CLIP Panic: {}", e))??;

        let img_noise = flux::sampling::get_noise(1, height, width, &self.device)?.to_dtype(dtype)?;
        ...
        let img = ((decoded.clamp(-1f32, 1f32)? + 1.0)? * 127.5)?.to_dtype(candle_core::DType::U8)?;
