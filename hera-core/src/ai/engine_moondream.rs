use crate::ai::{
    ChatChoice, ChatRequest, ChatResponse, ChatResponseMessage, ChatStreamResponse, ChatUsage,
    ContentPart, InferenceError, LLMEngine, MessageContent,
};
...
impl MoondreamFfiEngine {
    pub async fn new() -> Result<Self, String> {
        ...
        info!(
            "👁️ [Vision] Loading Moondream quantized model into {:?}...",
            device
        );
        let vb =
    ...
    fn load_image_from_base64(&self, b64: &str) -> Result<Tensor, String> {
        ...
        let normalized = (data_tensor.to_dtype(candle_core::DType::F32).unwrap() / 255.)
            .unwrap()
            .broadcast_sub(&mean)
            .unwrap()
            .broadcast_div(&std)
...
impl LLMEngine for MoondreamFfiEngine {
    async fn generate_content(&self, req: ChatRequest) -> Result<ChatResponse, InferenceError> {
        ...
        let image_tensor = if !base64_image.is_empty() {
            ...
            let encoded = image_embeds.apply(model.vision_encoder()).map_err(|e| {
                InferenceError::ExecutionFailed(format!("Vision encoder error: {}", e))
            })?;
            Some(encoded)
        } else {
            None
        ...
        let mut logits_processor = candle_transformers::generation::LogitsProcessor::new(
            299792458,
            req.temperature.map(|t| t as f64).filter(|&t| t > 0.0),
            None,
        );
        ...
        let initial_input = Tensor::new(token_ids.as_slice(), &self.device)
            .unwrap()
            .unsqueeze(0)
            .unwrap();

        let mut logits = if let Some(embeds) = &image_tensor {
            let bos_tensor = Tensor::new(&[special_token], &self.device)
                .unwrap()
                .unsqueeze(0)
                .unwrap();
            model
                .text_model
                .forward_with_img(&bos_tensor, &initial_input, embeds)
                .unwrap()
        } else {
