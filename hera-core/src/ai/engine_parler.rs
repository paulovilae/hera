...
impl ParlerEngine {
    pub fn new() -> Result<Self> {
        ...
        let repo = api.repo(hf_hub::Repo::model("parler-tts/parler-tts-mini-v1".to_string()));
    ...
    pub async fn synthesize_speech(
        ...
        let desc_tokens = self.tokenizer
        ...
        let pcm_tensor = model_lock.audio_encoder.decode_codes(&codes.to_device(&self.device)?)?;
        ...
        let mut cursor = Cursor::new(Vec::new());
        {
            ...
            for sample in normalized_pcm {
                writer.write_sample(sample).context("Failed to write audiom sample")?;
            }
