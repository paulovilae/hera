...
impl WhisperEngine {
    pub fn new() -> Result<Self> {
        ...
        let vb = candle_transformers::quantized_var_builder::VarBuilder::from_gguf(&weights_file, &device)?;
    ...
    pub async fn transcribe_audio(&self, wav_bytes: &[u8]) -> Result<String> {
        ...
        if sample_rate != m::SAMPLE_RATE as u32 {
            anyhow::bail!("Input file must have a {} sampling rate, but got {}", m::SAMPLE_RATE, sample_rate);
        }
        ...
        let language_token: Option<u32> = None; // For tiny-en, English is default.
        ...
        while seek < content_frames {
            ...
            for i in 0..sample_len {
                ...
                let ys = model_lock.decoder.forward(&tokens_t, &audio_features, i == 0)?;

                let (_, seq_len, _) = ys.dims3()?;
                let logits = model_lock.decoder.final_linear(&ys.i((..1, seq_len - 1..))?)?.i(0)?.i(0)?;
            ...
            let segment_text = self.tokenizer.decode(&tokens, true).map_err(anyhow::Error::msg)?;
