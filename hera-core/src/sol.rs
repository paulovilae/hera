...
impl SolParser {
    ...
    pub fn parse(input: &str) -> Result<SolOperation, String> {
        ...
        match tokens[0].to_uppercase().as_str() {
            "INSPECT" => {
                if tokens.len() == 2 {
                    Ok(SolOperation::Inspect { object_id: tokens[1].to_string() })
                } else {
            ...
            "EXTRACT" => {
                if tokens.len() == 4 && tokens[2].to_uppercase() == "FROM" {
                    Ok(SolOperation::Extract { field: tokens[1].to_string(), object_id: tokens[3].to_string() })
                } else {
                    Err("Syntax error: EXTRACT <field> FROM <object_id>".to_string())
                }
            }
            "FIND" => {
                 if tokens.len() == 4 && tokens[1].to_uppercase() == "CHILDREN" && tokens[2].to_uppercase() == "OF" {
                     Ok(SolOperation::FindChildren { parent_id: tokens[3].to_string() })
                 } else {
            ...
            "JOIN" => {
                if tokens.len() == 4 && tokens[2].to_uppercase() == "WITH" {
                    Ok(SolOperation::Join { left_id: tokens[1].to_string(), right_id: tokens[3].to_string() })
                } else {
            ...
            "INSERT" => {
                if let Some(into_idx) = tokens.iter().rposition(|&t| t.to_uppercase() == "INTO") {
                    if into_idx > 1 && into_idx + 1 < tokens.len() {
            ...
            "QUERY" => {
                ...
                if tokens.len() >= 5 && tokens[1].to_uppercase() == "SYSTEM" {
                     let with_idx = tokens.iter().position(|&t| t.to_uppercase() == "WITH").unwrap_or(0);
                     if with_idx > 2 {
...
pub fn execute_sol(op: &SolOperation) -> String {
    match op {
        SolOperation::Inspect { object_id } => {
            format!("[S.O.L Execution] INSPECT {}: {{ \"id\": \"{}\", \"type\": \"mock_object\", \"status\": \"inspected\" }}", object_id, object_id)
        }
        SolOperation::Extract { field, object_id } => {
            format!("[S.O.L Execution] EXTRACT {} FROM {}: \"mock_{}_value\"", field, object_id, field)
        }
        SolOperation::Join { left_id, right_id } => {
            format!("[S.O.L Execution] JOIN {} WITH {}: {{ \"joined\": true, \"left\": \"{}\", \"right\": \"{}\" }}", left_id, right_id, left_id, right_id)
        }
        SolOperation::Insert { content, object_id } => {
            format!("[S.O.L Execution] INSERT INTO {}: Successfully inserted content into object.", object_id)
        }
        SolOperation::FindChildren { parent_id } => {
            format!("[S.O.L Execution] FIND CHILDREN OF {}: [\"{}_child_1\", \"{}_child_2\"]", parent_id, parent_id, parent_id)
        }
        SolOperation::QuerySystem { database, query } => {
            format!("[S.O.L Execution] QUERY SYSTEM {}: Executed SQL -> \"{}\"", database, query)
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn test_inspect() {
        assert_eq!(SolParser::parse("INSPECT obj_contract_123").unwrap(), 
                   SolOperation::Inspect { object_id: "obj_contract_123".to_string() });
    }

    #[test]
    fn test_query_system() {
        assert_eq!(SolParser::parse("QUERY SYSTEM latinos_db WITH SELECT * FROM trades").unwrap(), 
                   SolOperation::QuerySystem { database: "latinos_db".to_string(), query: "SELECT * FROM trades".to_string() });
    }
}
