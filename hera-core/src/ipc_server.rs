                            ...
                            Ok(n) if n > 0 => {
                                ...
                                match serde_json::from_slice::<IpcPayload>(&buffer) {
                                    Ok(request) => {
                                        ...
                                        let mut tool_execution_success: Option<bool> = None;
                                        ...
                                        if request.action == "execute_tool" {
                                            ...
                                            } else if !permissions.contains(&"all".to_string())
                                                && !permissions.contains(&tool_name)
                                            {
                                                result_text = format!(
                                                    "Error: Tool '{}' denied by permissions",
                                                    tool_name
                                                );
                                            } else {
                                                let tool_call =
                                                ...
                                                let tool_result =
                                                    crate::ai::tool_executor::execute_tool(
                                                        &tool_call,
                                                    )
                                                    .await;
                                                response_origin = "tool".to_string();
                                                response_model = tool_name.clone();
                                                tool_execution_success = Some(tool_result.success);
                                            ...
                                            if prompt.is_empty() {
                                                if let Some(messages) = payload_clone
                                                    .get("messages")
                                                    .and_then(|m| m.as_array())
                                                {
                                                    if let Some(last_msg) = messages.last() {
                                                        if let Some("user") = last_msg
                                                            .get("role")
                                                            .and_then(|r| r.as_str())
                                                        {
                                                    ...
                                                    if messages.len() >= 2 {
                                                        if let Some(prev_msg) =
                                                            messages.get(messages.len() - 2)
                                                        {
                                                            if let Some("assistant") = prev_msg
                                                                .get("role")
                                                                .and_then(|r| r.as_str())
                                                            {
                                                                if let Some(content) = prev_msg
                                                                    .get("content")
                                                                    .and_then(|c| c.as_str())
                                                                {
                                                                    assistant_last =
                                            ...
                                            let permissions: Vec<String> = payload_clone
                                                .get("permissions")
                                            ...
                                            if !handled_by_tool {
                                                ...
                                                let mut chat_req: Option<ChatRequest> =
                                                    serde_json::from_value(payload_clone.clone())
                                                        .ok();

                                                if chat_req.is_none() {
                                                    if !prompt.is_empty() {
                                                        let app_name = payload_clone
                                                            .get("app")
                                                            .and_then(|v| v.as_str())
                                                            .unwrap_or("")
                                                            .to_string();
                                                    ...
                                                    let agent_identity =
                                                        std::path::Path::new(&persona_path)
                                                            ...
                                                            .unwrap_or("unknown");
                                                    let schemas =
                                                        crate::ai::tool_executor::hera_tool_schemas(
                                                            &permissions,
                                                            agent_identity,
                                                        );
                                                    let db_schema_ctx = fetch_db_schema_context(
                                                        agent_identity,
                                                        &app_name,
                                                    )
                                                    .await;
                                                                ...
                                                                resp.choices.first()
                                                            {
                                                                    ...
                                                                    &choice.message.content
                                                                {
                                                                    ...
                                                                    if !parsed_calls.is_empty() {
                                                                        ...
                                                                        let mut execution_outputs =
                                                                            String::new();
                                                                        let mut executed_calls =
                                                                            Vec::new();
                                            ...
                                            tracing::info!(
                                                "🛡️ [Hera IPC Stream] Parsed permissions: {:?}",
                                                permissions
                                            );
                                            ...
                                            let mut chat_req: Option<crate::ai::ChatRequest> =
                                            ...
                                            if chat_req.is_none() {
                                                ...
                                                if let Some(first) = req.messages.first_mut() {
                                                    if first.role == "system" {
                                                        match &mut first.content {
                                                            crate::ai::MessageContent::Text(t) => {
                                                                *t = format!(
                                                                    "{}\n\n{}",
                                                                    full_system_prompt, t
                                                                );
                                                            }
                                                            crate::ai::MessageContent::Parts(
                                                                parts,
                                                            ) => {
                                                                parts.insert(
                                                                    0,
                                                                    crate::ai::ContentPart::Text {
                                                                        text: format!(
                                                                            "{}\n\n",
                                                                            full_system_prompt
                                                                        ),
                                                    ...
                                                    Ok(mut rx) => {
                                                        while let Some(chunk_res) = rx.recv().await
                                                        {
                                                            if let Ok(chunk) = chunk_res {
                                                                ...
                                                                if !buffer_flushed {
                                                                    let trimmed = final_result_text
                                                                        .trim_start();
                                                                    // Check for actual tool call patterns, NOT <think> tags
                                                                    let looks_like_tool = trimmed
                                                                        .starts_with('{')
                                                                        || trimmed.starts_with(
                                                                            "<tool_call>",
                                                                        )
                                                        ...
                                                        if !parsed_calls.is_empty() {
                                                            ...
                                                            if !has_media_call {
                                                                ...
                                                                content: crate::ai::MessageContent::Text(sys_msg),
                                                                    ...
                                                                    if let Ok(mut rx2) = state
                                                                        .engine
                                                                        .generate_stream(req2)
                                                                        .await
                                                                    {
                                                                        while let Some(chunk_res2) =
                                                                            rx2.recv().await
                                                                        {
                                                                            if let Ok(chunk2) =
                                                                                chunk_res2
                                                        ...
                                                        dstr.push('\n');
                                                        let _ =
                                                            stream.write_all(dstr.as_bytes()).await;
                                                        break;
                                                    }
                                                    Err(e) => {
                                            ...
                                            if let Some(prompt_val) = request
                                                .payload
                                                .get("prompt")
                                                .and_then(|p| p.as_str())
                                            {
                                                ...
                                                {
                                                    if let Ok(triggers) = serde_json::from_str::<
                                                    ...
                                                    >(
                                                        ...
                                                        let mut claimed_keywords =
                                                            std::collections::HashSet::new();

                                                        // 1. Explicit LoRAs claim their auto-triggers first
                                                        for (lora_name, keywords) in &triggers {
                                                            if prompt_lower.contains(&format!(
                                                                "<lora:{}",
                                                                lora_name.to_lowercase()
                                                            )) {
                                                                for k in keywords {
                                                                    claimed_keywords
                                                                        .insert(k.to_lowercase());
                                                                }
                                                            }
                                                        }
                                                ...
                                                .await;
                                                    ...
                                                    match response {
                                                        ...
                                                        Err(e) => {
                                                            error!(
                                                                "sd.cpp connection error: {}",
                                                                e
                                                            );
                                                            result_text = format!(
                                                                "Error connecting to Native Image Generator: {}",
                                                                e
                                                            );
                                                        }
                                                    }
                                            ...
                                            || request.action == "animate_image"
                                        {
                                                ...
                                                .and_then(|p| p.as_str())
                                            {
                                                ...
                                                let enhanced = match state
                                                    .engine
                                                    .generate_content(chat_req)
                                                    .await
                                                {
                                                    Ok(resp) => resp
                                                        .choices
                                                    ...
                                                    Err(e) => {
                                                        error!(
                                                            "Brain prompt enhancement failed: {}, using raw prompt",
                                                            e
                                                        );
                                                        prompt.to_string()
                                                    }
                                                ...
                                                let anchor_image_b64: Option<String> = if let Some(
                                                    img,
                                                ) =
                                                    user_image_b64
                                                {
                                                    info!(
                                                        "🖼️ Using user-provided image as anchor frame"
                                                    );
                                                    Some(img)
                                                } else {
                                                ...
                                                .await
                                            {
                                                ...
                                                info!(
                                                    "🔄 GPU Swap: Stopping FLUX to free VRAM for video generation..."
                                                );
                                                ...
                                                if let Some(ref b64_img) = anchor_image_b64 {
                                                    canvas_payload["image_base64"] =
                                                        serde_json::Value::String(b64_img.clone());
                                                    info!(
                                                        "📹 Sending anchor image to VACE I2V pipeline"
                                                    );
                                                } else {
                                                    info!(
                                                        "📹 Using T2V pipeline (no anchor image)"
                                                    );
                                                }
                                                    ...
                                                    .await
                                                {
                                                    ...
                                                    Err(e) => {
                                                        ...
                                                        result_text = format!(
                                                            "Error: Canvas video engine unavailable: {}",
                                                            e
                                                        );
                                                    }
                                                }
                                                ...
                                                info!(
                                                    "🔄 GPU Swap: Restarting FLUX after video generation..."
                                                );
                                                let _ = tokio::process::Command::new("pm2")
                                            ...
                                            let civitai_token = std::fs::read_to_string(
                                                "/home/paulo/Programs/apps/OS/Imaginclaw/.env",
                                            )
                                            .ok()
                                            .and_then(|content| {
                                                content
                                            ...
                                            if url.is_empty() {
                                                ...
                                                result_text = "❌ CIVITAI_TOKEN not found in .env"
                                                    .to_string();
                                            } else {
                                                // Extract model ID from CivitAI page URL: https://civitai.com/models/123456/...
                                                let model_id_opt: Option<String> =
                                                    if url.contains("civitai.com/models/") {
                                                        ...
                                                        .await
                                                    {
                                                            ...
                                                            if api_resp.status().is_success() =>
                                                        {
                                                                ...
                                                                .await
                                                            {
                                                                            ...
                                                                            Ok(bytes) => {
                                                                                match std::fs::write(&dest_path, &bytes) {
                                                                                    Ok(_) => {
                                                                                                ...
                                                                                                if let Some(ts) = t.as_str() {
                                                                                                    for part in ts.split(',') {
                                                                                                        ...
                                                                                                        if !trimmed.is_empty() && trimmed.chars().count() <= 40 {
                                                                                                            auto_tags.push(trimmed.to_string());
                                                                                                        }
                                                                                                    }
                                                                                                }
                                                                                            }
                                                                                        ...
                                                                                        // Append to nsfw_loras.json if flagged
                                                                                        let mut is_nsfw = model_data.get("nsfw").and_then(|v| v.as_bool()).unwrap_or(false);
                                                                                        if !is_nsfw {
                                                                                            if let Some(level) = model_data.get("nsfwLevel").and_then(|v| v.as_i64()) {
                                                                                                if level > 1 { is_nsfw = true; }
                                                                                            }
                                                                                        }

                                                                                        if is_nsfw {
                                                                                            let nsfw_path = format!("{}/nsfw_loras.json", lora_dir);
                                                                                            let mut nsfw_tags: Vec<String> = if let Ok(content) = std::fs::read_to_string(&nsfw_path) {
                                                                                                serde_json::from_str(&content).unwrap_or_default()
                                                                                            } else {
                                                                                                Vec::new()
                                                                                            };
                                                                                            if !nsfw_tags.contains(&tag_name) {
                                                                                                nsfw_tags.push(tag_name.clone());
                                                                                                if let Ok(new_content) = serde_json::to_string_pretty(&nsfw_tags) {
                                                                                                    let _ = std::fs::write(&nsfw_path, new_content);
                                                                                                }
                                                                                            }
                                                                                            auto_tags.push("NSFW".to_string());
                                                                                        }
                                                                ...
                                                                Err(e) => {
                                                                    result_text = format!(
                                                                        "❌ Failed to parse CivitAI API response: {}",
                                                                        e
                                                                    )
                                                                }
                                                            }
                                                        ...
                                                        Ok(api_resp) => {
                                                            result_text = format!(
                                                                "❌ CivitAI API error: HTTP {}",
                                                                api_resp.status()
                                                            )
                                                        }
                                                        Err(e) => {
                                                            result_text = format!(
                                                                "❌ CivitAI API request failed: {}",
                                                                e
                                                            )
                                                        }
                                                    }
                                        ...
                                        if let Some(map) = data_json.as_object_mut() {
                                            map.insert(
                                                "origin".to_string(),
                                                serde_json::json!(response_origin),
                                            );
                                            map.insert(
                                                "model".to_string(),
                                                serde_json::json!(response_model),
                                            );
                                            if let Some(success) = tool_execution_success {
                                                map.insert(
                                                    "success".to_string(),
